...
exports.getWholesalerProductsWithReviews = async (req, res) => {
  try {
    const wholesalerId = req.user._id;

    // Get reviews where createdBy matches the wholesaler ID
    const reviewStats = await Review.aggregate([
      {
        $match: { createdBy: wholesalerId }
      },
      {
        $group: {
          _id: '$purchaseId',
    ...
    console.error('Error fetching wholesaler products with reviews:', error);
    res.status(500).json({
      success: false,
...
exports.getWholesalerProductReviews = async (req, res) => {
  try {
    ...
    // Get all reviews for these purchases where createdBy matches wholesaler
    ...
    console.error('Error fetching wholesaler product reviews:', error);
    res.status(500).json({
      success: false,
